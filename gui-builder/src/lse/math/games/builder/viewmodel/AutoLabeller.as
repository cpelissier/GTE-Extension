	...
	public class AutoLabeller
	{
		...
		public function AutoLabeller(alpha:Vector.<String> =  null)
		{
			if(alpha != null) {
				...
				//trace("Updating autolabel lists");
		...
		public function getNextAutoLabel_Player1(game:Game):String
		{
			...
			trace("count1_p1 " + count1_p1);
			trace("count2_p1 " + count2_p1);
		...
		//CMP: Just use P1 counts. In this way P1 and P3 use alternating capitals from A-Z. 
		//e.g.: P1's first moves are A and B, then P3's first moves are C and D, then P1's next moves are E and F.
		public function getNextAutoLabel_Player3(game:Game):String
		{
			var label:String = _alpha[count1_p1] + (count2_p1 != _alpha.length ? _alpha[count2_p1] : "");
			...
			while (bfs.length>0) {
				...
				else 
				{
					...
					while (true) 
					{
						...
						{
							trace("Labelling player 1's moves");
							autoLabel = getNextAutoLabel_Player1(tree);
						} 
						else if (player==tree.firstPlayer.nextPlayer) 
						{
							trace("Labelling player 2's moves");
							autoLabel = getNextAutoLabel_Player2(tree);
						}
						else if(player==tree.firstPlayer.nextPlayer.nextPlayer)
						{
							trace("Labelling player 3's moves");
							autoLabel = getNextAutoLabel_Player3(tree);
						}
